...
test('boot 链一条龙：applyAll(54) → composition-integrity → preflight → 二遍幂等', { skip: !hasPristine() && '缺 .tmp-rc2-stage pristine 源' }, async (t) => {
  const { appDir, home, ctx } = await buildTempRoots(t);

  // ---- 1. 一遍 applyAll：54 补丁全执行、有落盘、零 errors ----
  const r1 = applyAll(ctx);
  assert.equal(r1.total, 54, `注册表应有 54 个补丁（实际 ${r1.total}）`);
  assert.equal(PATCH_SPECS.length, 54, 'PATCH_SPECS 与编排 total 一致');
